...
**A\*和 IDA\*算法关键在于设计估值函数**，当估值函数过于乐观时（剪枝的效果微乎其微），当估值函数不乐观时，算法则会出错。估值函数的设计思路：
- 考虑一次操作能达到的最好效果是什么，从而估计至少需要多少次操作（例题，排数、回转游戏）。
- 有时情况比较复杂，如果考虑每次操作最好能达到什么效果，设计出来的估值函数过于乐观。此时需要结合当前状态来动态的分析（例题，破坏正方形）。
