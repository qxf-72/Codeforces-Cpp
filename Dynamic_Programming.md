...
使用倍增优化 DP 解决问题时，一般分为两个部分：
- 预处理：用"阶段"成倍增长的 DP，计算出若干个与 2 的整数次幂相关的代表状态。
- 拼凑：基于二进制拆分的思想，用上一步得到的代表状态组合成答案。


### 例题

[**293. 开车旅行 - AcWing题库**](https://www.acwing.com/problem/content/295/)

本题的两个问题可以转化为一个问题——从每一个城市出发行驶 $X$ 总里程，两个人各自驾驶的距离是多少。

首先，从特定城市 $S_i$ 出发，行驶 $X$ 公里的终点是确定了，对于如何确定终点，有两种策略：
- 二分法：对于二分能行驶的天数进行二分，对于从一个特定城市 $S_i$ 出发，算法时间复杂度为 $O(n\times log\ n)$ ，总的时间复杂度为 $O(n^2\times log\ n)$ 。
- 倍增法：使用二进制拆分的方式确定能行驶的最大天数，需要进行 $O(n\times log\ n)$ 预处理，单次计算的时间复杂度为 $O(log\ n)$ 。总的时间复杂度为 $O(n\times log\ n)$ 。

所以考虑倍增的方法，首先需要预处理：
- $ga(i)$ 和 $gb(i)$ ：两个人从 $S_i$ 出发的下一个城市。
- $f[i,j,k]$ ：从城市 $S_j$ 出发， $k$ 先开车，行驶 $2^i$ 天到达的城市。
- $da[i,j,k]$ ：从城市 $S_j$ 出发， $k$ 先开车，行驶 $2^i$ 天，小 A 行驶的路程总和。
- $db[i,j,k]$


在计算从城市 $S$ 出发最多行驶 $X$ 公里，二人各自行驶的路程时，按照递减的顺序枚举所有 $2$ 的整数次幂，作为行驶天数：
- 初始化当前城市 $p=S$ ， $la=0,lb=0$
- 倒序循环 $i=log \ n$ 到 $0$
- 对于每一个 $i$ ，如果从 $p$ 出发行驶 $2^i$ 天，总行驶里程不超过 $X$ ，则 $p=f[i,p,0]$ ， $la=la+da[i,p,0]$ ， $lb=lb+db[i,p,0]$ 。这里需要注意，由于是倒序枚举 $i$ ，所以当前已经行驶天数必然是偶数，所以每次都是 $A$ 先开车。

<br/>

[**294. 计算重复 - AcWing题库**](https://www.acwing.com/problem/content/296/)

首先 $conn(conn(s_2，n_2)，m)=conn(s_2,n_2\times m)$ ，将问题转化为求一个最大的 $m$ ，满足 $conn(s_2,m)$ 可以由 $conn(s_1,n_1)$ 表示。

首先可以确定的是 $|s_2|\times m\le |s_1|\times n_1$ ，由此可以确定 $m$ 的上界，可以对于 $m$ 进行二进制枚举。

设 $F[i,j]$ 表示从 $s_1[i]$ 开始表示 $conn(s_2,2^j)$ 至少需要多少个字符。可以得到状态转移方程：

$$
F\left[ i,j \right] =F\left[ i,j-1 \right] +F\left[ \left( i+F\left[ i,j-1 \right] \right) \ mod\ |s_1| \right] \left[ j-1 \right] 
$$

对于初值 $F[i,0]$ 可以使用朴素方法直接计算。
...
DP 在进行状态转移时，如果需要枚举一个决策，在所有决策中选最大值或最小值，如果这个决策的集合只扩大，不缩小，可以仅仅使用一个变量进行维护。

在更普遍的情况下，需要使用数据结构进行维护 DP 决策的候选集合，以便快速执行插入元素、删除元素、查询最值等操作。

### 例题

[**295. 清理班次 - AcWing题库**](https://www.acwing.com/problem/content/297/)
...
[**296. 清理班次2 - AcWing题库**](https://www.acwing.com/problem/content/298/)

设 $f[x]$ 为覆盖 $[L,x]$ 花费的最小代价。

将所有贴纸按照区间右端点 $b_i$ 递增排序，按照顺序扫描这些贴纸，设当前贴纸为 $[a_i,b_i]$ ，则有

$$
f\left[ b_i \right] =\underset{a_i-1\le x<b_i}{\min}\left\{ f\left[ x \right] \right\} +c_i
$$

可以看出，决策集合为 $a_i-1$ 到 $b_i$ ，取其中 $f[]$ 最小的一个，在遍历贴纸时，决策集合的上下界变化没有特别大规律性，所以可以使用比较灵活的线段树来维护。
